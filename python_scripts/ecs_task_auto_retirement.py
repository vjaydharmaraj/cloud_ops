...
slack_token = os.getenv("SLACK_WEBHOOK_URL")                        # Make sure to add the env for Slack token
channel = "#test-vijay"                                             # Test slack channel name
...
def refresh_ecs_service(cluster, service):
    ...
    try:
        start_time = datetime.now()                                             # Starting time of the service refresh
...
def wait_for_service_stabilization(cluster, service, timeout=300):              # wait time to stabilze the task
...
def process_event(event_arn):
    ...
    if not affected_entities:
        return                                                          # Loop will break if there is no affected entities
